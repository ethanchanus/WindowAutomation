...
namespace Win32Interop.WinHandles
{
    ...
    public static class ChildWindowUtils
    {
        ...
        [NotNull]
        public static IEnumerable<WindowHandle> FindChildWindows(IEnumerable<WindowHandle> parentWndHdlList, Predicate<WindowHandle> windowPredicate)
        {
            if (windowPredicate == null)
                throw new ArgumentNullException(nameof(windowPredicate));

            List<WindowHandle> windows = null;
            foreach (var parentWndHdl in parentWndHdlList)
            {
                NativeMethods.EnumChildWindows(parentWndHdl.RawPtr, (ptr, param) =>
                {
